...
router.post("/", isAuthorized, isWalletVerified, rampWidgetController.createRampSession);
...
export async function createRampWidget(request: CreateRampWidgetRequest): Promise<ZBDRampWidgetResponse> {
  try {
    ...
    return handleZbdApiError(err, `createRampWidget(${JSON.stringify(request)})`);
  }
}
```
